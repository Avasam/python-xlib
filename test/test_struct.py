...
from test import DummyDisplay, TestCase
...
class StructTest(object):
    ...
    def test_pack_value_dict(self):
        self.assertBinaryEqual(
            self.struct.pack_value(dict(self.values_in)), self.binary
        )

    def test_pack_value_tuple(self):
        self.assertBinaryEqual(
            self.struct.pack_value(tuple(self.values_in.values())), self.binary
        )

    def test_to_binary_args(self):
        self.assertBinaryEqual(
            self.struct.to_binary(*self.values_in.values()), self.binary
        )

    def test_to_binary_kwargs(self):
        self.assertBinaryEqual(
            self.struct.to_binary(**dict(self.values_in)), self.binary
        )
...
def _struct_test(name, fields):
    ...
    class_name = "".join(
        [part.capitalize() for part in re.sub(r"[^\w]+", " ", name).split()]
    )
    ...
    for field_params in fields:
        ...
        if isinstance(field_binary, binary_type):
            ...
            raise ValueError(
                "unsupported type for binary: {data} [{type}]".format(
...
_struct_test(
    ...
    (
        ...
        (
            "lc8",
            lambda name: rq.List(name, rq.Card8, pad=False),
            [0x42, 0xC3],
            lambda v: pack("2B", *v),
        ),
        (
            "lc16",
            lambda name: rq.List(name, rq.Card16, pad=False),
            [666, 1, 60143],
            lambda v: pack("3H", *v),
        ),
        (
            "lc32",
            lambda name: rq.List(name, rq.Card32, pad=False),
            [0xF02FACB, 666, 1, 0x1043, 0xDEADBEEF],
            lambda v: pack("5L", *v),
        ),
    ),
)
...
_struct_test(
    ...
    (
        ...
        (
            "lc8",
            lambda name: rq.List(name, rq.Card8, pad=True),
            [0x42, 0xC3],
            lambda v: pack("2B2x", *v),
        ),
        (
            "lc16",
            lambda name: rq.List(name, rq.Card16, pad=True),
            [666, 1, 60143],
            lambda v: pack("3H2x", *v),
        ),
        (
            "lc32",
            lambda name: rq.List(name, rq.Card32, pad=True),
            [0xF02FACB, 666, 1, 0x1043, 0xDEADBEEF],
            lambda v: pack("5L", *v),
        ),
    ),
...
_struct_test(
    ...
    (
        (
            "lc8",
            lambda name: rq.FixedList(name, 2, rq.Card8, pad=False),
            [0x42, 0xC3],
            lambda v: pack("2B", *v),
        ),
        (
            "lc16",
            lambda name: rq.FixedList(name, 3, rq.Card16, pad=False),
            [666, 1, 60143],
            lambda v: pack("3H", *v),
        ),
        (
            "lc32",
            lambda name: rq.FixedList(name, 5, rq.Card32, pad=False),
            [0xF02FACB, 666, 1, 0x1043, 0xDEADBEEF],
            lambda v: pack("5L", *v),
        ),
    ),
...
_struct_test(
    ...
    (
        (
            "lc8p",
            lambda name: rq.FixedList(name, 2, rq.Card8, pad=True),
            [0x42, 0xC3],
            lambda v: pack("2B2x", *v),
        ),
        (
            "lc16p",
            lambda name: rq.FixedList(name, 3, rq.Card16, pad=True),
            [666, 1, 60143],
            lambda v: pack("3H2x", *v),
        ),
        (
            "lc32p",
            lambda name: rq.FixedList(name, 5, rq.Card32, pad=True),
            [0xF02FACB, 666, 1, 0x1043, 0xDEADBEEF],
            lambda v: pack("5L", *v),
        ),
    ),
...
_struct_test(
    ...
    (
        ...
        ("s1", lambda name: rq.String8(name, pad=False), "testing", packstr),
        ("s2", lambda name: rq.String8(name, pad=False), "one two three", packstr),
        (
            "s3",
            lambda name: rq.String8(name, pad=False),
...
_struct_test(
    ...
    (
        ...
        (
            "s1",
            lambda name: rq.String8(name, pad=True),
            "testing",
...
_struct_test(
    ...
    (
        ...
        (
            "s1",
            lambda name: rq.String16(name, pad=False),
            (0, 1, 2),
        ...
        (
            ...
            lambda name: rq.String16(name, pad=False),
            b"\x03\x04\x05",
            lambda s: struct.pack(">" + "H" * len(s), *iterbytes(s)),
            (3, 4, 5),
        ),
    ),
)
...
_struct_test(
    ...
    (
        ...
        (None, lambda name: rq.LengthOf("s2", 2), None, pack("H", 13)),
        (None, lambda name: rq.LengthOf("s3", 4), None, pack("L", 34)),
        ("s1", lambda name: rq.Binary(name, pad=False), b"testing", lambda v: v),
        ("s2", lambda name: rq.Binary(name, pad=False), b"one two three", lambda v: v),
        (
            "s3",
            lambda name: rq.Binary(name, pad=False),
...
_struct_test(
    ...
    (
        ...
        (
            "s1",
            lambda name: rq.Binary(name, pad=True),
            b"testing",
            lambda v: v + b"\0" * 1,
        ),
        (
...
_struct_test(
    ...
    (
        ...
        (
            "s3",
            lambda name: rq.FixedBinary(name, 34),
            b"supercalifragilisticexpialidocious",
            lambda v: v,
        ),
